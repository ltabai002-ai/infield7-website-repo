...
export function LegacySections({ onDemo }: { onDemo: () => void }) {
  ...
  return <div ref={root} className="legacy-sections">{legacySections.map(section=><div data-react-section key={section.id} dangerouslySetInnerHTML={{__html:section.html}}/>)}</div>;
}
export function LegacyFooter(){ return <div dangerouslySetInnerHTML={{__html:legacyFooterHtml}}/>;
}
